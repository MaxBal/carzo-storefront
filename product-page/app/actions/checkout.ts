...
import { resolveNovaPoshtaPoint } from '@/lib/nova-poshta';
import { notifyNewOrder } from '@/lib/order-notifications';
import { contactMethodLabel } from '@/lib/cart/contact-method';
import type { CheckoutInput, CheckoutResult } from '@/lib/cart/types';
...
export async function createOrder(input: CheckoutInput): Promise<CheckoutResult> {
  ...
  try {
    ...
    const delivery = await resolveNovaPoshtaPoint(parsed.data.deliveryCityRef, parsed.data.deliveryPointRef);
    if (delivery.point.type === 'postomat' && !quote.allowPostomat) {
    ...
    await notifyNewOrder({
      ...
      deliveryPoint: delivery.point.name,
