  ...
  return (
    <footer className="bg-black text-white">
      ...
      <div className="hidden md:block">
        <div className="mx-auto max-w-[1280px] px-4 py-12 lg:px-8">
          <div className="grid grid-cols-5 gap-8">
            {/* Column 1: Каталог */}
            <div>
              <h3 className="mb-4 text-[15px] font-semibold uppercase tracking-wider text-white">Каталог</h3>
              <ul className="space-y-3">
                {catalogLinks.map(link => (
                  <li key={link.label}>
                    <Link href={link.href} className="text-[15px] font-medium text-white transition-colors hover:text-[#5ce4ab]">
                      {link.label}
                    </Link>
                  </li>
                ))}
            ...
            {/* Column 2: Покупцеві */}
            <div>
              <h3 className="mb-4 text-[15px] font-semibold uppercase tracking-wider text-white">Покупцеві</h3>
              <ul className="space-y-3">
                  ...
                  <li key={link.label}>
                    {link.action === 'link' ? (
                      ...
                      <a
                        ...
                        className="text-[15px] font-medium text-white transition-colors hover:text-[#5ce4ab]"
                      >
                        {link.label}
                      </a>
                    )}
            ...
            {/* Column 3: B2B */}
            <div>
              <h3 className="mb-4 text-[15px] font-semibold uppercase tracking-wider text-white">B2B</h3>
              <ul className="space-y-3">
                {b2bLinks.map(link => (
                  <li key={link.label}>
                    <a
            ...
            <div>
              <h3 className="mb-4 text-[15px] font-semibold uppercase tracking-wider text-white">Carzo bonus</h3>
              <ul className="space-y-3">
                {bonusLinks.map(link => (
                  <li key={link.label}>
                    <a
            ...
            {/* Column 5: Контакти */}
            <div>
              <h3 className="mb-4 text-[15px] font-semibold uppercase tracking-wider text-white">Зателефонувати</h3>
              <a href={CARZO_LINKS.phone} className="text-[16px] font-medium text-white transition-colors hover:text-[#5ce4ab]">
                +380 66 103 10 94
              </a>
              <p className="mt-1 text-[15px] text-white">з 10:00 до 20:00</p>
              
              <h3 className="mb-3 mt-6 text-[15px] font-semibold uppercase tracking-wider text-white">Приєднатися</h3>
      ...
      <div className="md:hidden">
        <div className="px-4 py-8">
          ...
          <div className="mt-6 space-y-3">
            {legalLinks.map(link => (
              <Link key={link.label} href={link.href} className="block text-[15px] font-medium text-white transition-colors hover:text-[#5ce4ab]">
                {link.label}
              </Link>
            ))}
          </div>
          ...
          {/* Copyright */}
          <p className="mt-6 text-center text-[12px] text-white">
      ...
      <div className="hidden md:block border-t border-white/10">
        <div className="mx-auto max-w-[1280px] px-4 py-5 text-center text-xs text-white lg:px-8">
          © 2026 Carzo всі права захищені
        </div>
      </div>
    </footer>
